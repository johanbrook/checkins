...
const deleteFriendQuery = e.params({ friendId: e.uuid, userId: e.uuid }, (params) =>
    e.delete(e.Friend, (c) => ({
        filter: e.op(e.op(c.user.id, '=', params.userId), 'and', e.op(c.id, '=', params.friendId)),
    })),
);
...
const deleteGroupQuery = e.params({ groupId: e.uuid, userId: e.uuid }, (params) =>
    e.delete(e.Grp, (c) => ({
        filter: e.op(e.op(c.user.id, '=', params.userId), 'and', e.op(c.id, '=', params.groupId)),
    })),
);
...
const findGroupQuery = e.params({ groupId: e.uuid, userId: e.uuid }, (params) =>
    e.select(e.Grp, (g) => ({
        filter: e.op(e.op(g.user.id, '=', params.userId), 'and', e.op(g.id, '=', params.groupId)),
    })),
);
...
export const createModel = (db: EdgeDb) => ({
    findGroups: (userId: string): Promise<FindGroups> => findGroupsQuery.run(db, { userId }),

    findGroupsForFeed: (userSlug: string) => findGroupsForFeedQuery.run(db, { userSlug }),
